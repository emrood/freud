...
@section('title') {{ 'View Blog | '.env('APP_NAME') }} @endsection

@section('breadcrumbs')
    @include('backend.layouts.partials.breadcrumbs',['current' => 'View Blog'])
@endsection
                            ...
                            <div class="row">
                                <div class="the-box no-border blog-detail-content col-md-12">
                                    ...
                                    <form method="post" class="bf" enctype="multipart/form-data"
                                          action="{{url('blog/'.$blog->id.'/storecomment')}}">
                                        {{csrf_field()}}
                                        <div class="row mb-3">
                                            <div class="form-group m-b-0 col-12 col-md-6 {{ $errors->has('name') ? 'has-error' : '' }}">
                                                <input id="name" type="text" name="name" class="form-control"
                                                       value="{{old('name')}}" placeholder="Enter Name">
                                                <span class="help-block">{{ $errors->first('name', ':message') }}</span>
                                            </div>
                                            <div class="form-group m-b-0 col-12 col-md-6 {{ $errors->has('email') ? 'has-error' : '' }}">
